...
export function renderDashboard(state: XuanMarketState, decision: BotDecision, nowTs: number): string {
  ...
  return [
    ...
    `pair_entries=${decision.entryBuys.filter((entryBuy) => entryBuy.reason !== "lagging_rebalance").length}`,
    `rebalance_entries=${decision.entryBuys.filter((entryBuy) => entryBuy.reason === "lagging_rebalance").length}`,
