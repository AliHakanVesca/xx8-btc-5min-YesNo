...
import {
  ...
  plannedOppositeLateSeedDeadlineFits,
...
export interface EntryDecisionTrace {
  ...
  plannedLowSideAvg?: number;
	  ...
	  plannedOppositeProtectiveRelease?: boolean;
...
function aggressiveOppositeReleaseHold(args: {
...
}): { holdSec: number; ageSec: number; force?: boolean | undefined } | undefined {
  ...
  const earlyLargeFamilyHighSideStagedSeed =
    ...
    stagedSeedMode;
  const lateHighSideStagedSeed =
    largeStagedBuy &&
    secsFromOpen >= 160 &&
    lastBuy.price >= 0.9 - 1e-9 &&
    stagedSeedMode;
  ...
  const waitSec = openingHighSideStagedSeed
    ...
    : lateHighSideStagedSeed
    ? Math.max(plannedOppositeMinWaitSec(args.config, secsFromOpen, args.secsToClose), 35)
...
function xuanTemporalSeedRhythmSkipReason(args: {
  ...
  const baseLot = Math.max(args.config.liveSmallLotLadder[0] ?? args.config.defaultLot, args.state.market.minOrderSize);
  const aggressiveSecondCampaignContinuation =
    isAggressivePublicFootprint(args.config) &&
    args.state.mergeHistory.length === 0 &&
    mergeableShares(args.state) >= baseLot * 2 - 1e-9 &&
    Math.abs(args.state.upShares - args.state.downShares) <= xuanFreshCycleFlatDustThreshold(args.config, args.state) + 1e-9 &&
    args.ctx.secsFromOpen >= 146 &&
    args.ctx.secsFromOpen <= 155 &&
    !sameSideSpam &&
    args.referencePairCost <= 1.06 + 1e-9;
  if (aggressiveSecondCampaignContinuation) {
    return { waitSec: 0, ageSec };
  }
  const strictOppositeStaging = isAggressivePublicFootprint(args.config) && !sameSideSpam;
  const aggressiveSmallHighLowContinuation =
    strictOppositeStaging &&
    args.state.mergeHistory.length === 0 &&
    mergeableShares(args.state) >= baseLot * 4 - 1e-9 &&
    Math.abs(args.state.upShares - args.state.downShares) <= xuanFreshCycleFlatDustThreshold(args.config, args.state) + 1e-9 &&
    args.ctx.secsFromOpen >= 158 &&
    args.ctx.secsFromOpen <= 180 &&
    args.referencePairCost <= 1.04 + 1e-9;
  const waitSec = strictOppositeStaging
    ? aggressiveSmallHighLowContinuation
      ? 7
...
function fixedFivePostMergeReentryCompletionBlockReason(args: {
  ...
  if (minCompletionShares > fixedFiveClip + 1e-9) {
    return "xuan_post_merge_reentry_uncloseable_fixed_clip";
  }
  if (projectedMissingOpposite > fixedFiveClip + args.config.maxCompletionOvershootShares + 1e-9) {
    return "xuan_post_merge_reentry_uncloseable_fixed_clip";
  }
  if (projectedMissingOpposite + args.config.maxCompletionOvershootShares < minCompletionShares - 1e-9) {
    return "xuan_post_merge_reentry_uncloseable_min_size";
  }

  return undefined;
}

function fixedFiveDirectionalSeedAllowed(
  config: XuanStrategyConfig,
  state: XuanMarketState,
  candidateContext: FreshCycleCandidateContext | undefined,
): boolean {
  const fixedFiveClip = fixedFiveEntryClipMax(config);
  if (
    fixedFiveClip === undefined ||
    !isAggressivePublicFootprint(config) ||
    candidateContext?.route !== "temporal_seed" ||
    candidateContext.side === undefined ||
    candidateContext.seedPrice === undefined ||
    candidateContext.oppositePrice === undefined ||
    candidateContext.effectivePair === undefined ||
    !Number.isFinite(candidateContext.seedPrice) ||
    !Number.isFinite(candidateContext.oppositePrice) ||
  ...
  if (
    candidateContext.requestedSize > fixedFiveClip + 1e-9 ||
    Math.abs(state.upShares - state.downShares) > xuanFreshCycleFlatDustThreshold(config, state) + 1e-9 ||
    candidateContext.ctx.secsToClose <= Math.max(config.finalWindowCompletionOnlySec, config.finalWindowNoChaseSec)
  ) {
    return false;
  }

  if (
    fixedFivePostMergeReentryCompletionBlockReason({
      config,
      state,
      side: candidateContext.side,
      seedSize: candidateContext.requestedSize,
    }) !== undefined
  ) {
    return false;
  }

  const snapshot = candidateContext.ctx.fairValueSnapshot;
  if (!snapshot || snapshot.status !== "valid") {
    return false;
  }
  const fairValue = fairValueForOrphanSide(snapshot, candidateContext.side);
  if (fairValue === undefined || !Number.isFinite(fairValue)) {
    return false;
  }

  const seedEffectivePrice =
    candidateContext.seedPrice + takerFeePerShare(candidateContext.seedPrice, config.cryptoTakerFeeRate);
  const highSideSeed = candidateContext.seedPrice >= candidateContext.oppositePrice - 1e-9;
  const spread = Math.abs(candidateContext.seedPrice - candidateContext.oppositePrice);
  const requiredFairValueEdge = highSideSeed ? 0.012 : 0.02;
  const maxDirectionalPairCost = Math.min(config.xuanBehaviorCap, 1.08);
  const openingOrRecycleWindow =
    candidateContext.ctx.secsFromOpen <= Math.max(config.campaignLaunchXuanProbeMaxAgeSec, 90) + 1e-9 ||
    state.mergeHistory.length > 0;

  return (
    openingOrRecycleWindow &&
    fairValue - seedEffectivePrice >= requiredFairValueEdge - 1e-9 &&
    candidateContext.effectivePair <= maxDirectionalPairCost + 1e-9 &&
    (highSideSeed || spread >= 0.08 - 1e-9)
  );
}

function fixedFiveDirectionalTemporalSeedTraceAllowed(args: {
  config: XuanStrategyConfig;
  state: XuanMarketState;
  ctx: EntryLadderContext;
  decision: EntryBuyDecision;
  seedTrace: SingleLegSeedCandidateTrace;
}): boolean {
  const fixedFiveClip = fixedFiveEntryClipMax(args.config);
  if (
    fixedFiveClip === undefined ||
    !isAggressivePublicFootprint(args.config) ||
    args.seedTrace.side !== args.decision.side ||
    args.decision.size > fixedFiveClip + 1e-9 ||
    args.seedTrace.filledSize < args.decision.size - 1e-9 ||
    args.seedTrace.oppositeCoverageRatio === undefined ||
    args.seedTrace.oppositeCoverageRatio < args.config.temporalSingleLegMinOppositeDepthRatio - 1e-9 ||
    args.seedTrace.effectivePair === undefined ||
    args.seedTrace.rawPair === undefined ||
    !Number.isFinite(args.seedTrace.effectivePair) ||
    !Number.isFinite(args.seedTrace.rawPair)
  ) {
    return false;
  }
  ...
  if (
    Math.abs(args.state.upShares - args.state.downShares) > xuanFreshCycleFlatDustThreshold(args.config, args.state) + 1e-9 ||
    args.ctx.secsToClose <= Math.max(args.config.finalWindowCompletionOnlySec, args.config.finalWindowNoChaseSec)
  ) {
    return false;
  }

  const snapshot = args.ctx.fairValueSnapshot;
  if (!snapshot || snapshot.status !== "valid") {
    return false;
  }
  const fairValue = fairValueForOrphanSide(snapshot, args.decision.side);
  if (fairValue === undefined || !Number.isFinite(fairValue)) {
    return false;
  }

  const oppositePrice = args.seedTrace.rawPair - args.seedTrace.averagePrice;
  const highSideSeed = args.seedTrace.averagePrice >= oppositePrice - 1e-9;
  const requiredFairValueEdge = highSideSeed ? 0.012 : 0.02;
  const maxDirectionalPairCost = Math.min(args.config.xuanBehaviorCap, 1.08);

  return (
    fairValue - args.seedTrace.effectivePricePerShare >= requiredFairValueEdge - 1e-9 &&
    args.seedTrace.effectivePair <= maxDirectionalPairCost + 1e-9
  );
}
...
function fixedFiveEntryClipMax(config: XuanStrategyConfig): number | undefined {
...
function fixedFiveResidualRepairClipMax(config: XuanStrategyConfig, shareGap: number): number | undefined {
  ...
  const overfillCloseAllowance = Math.max(1, config.maxCompletionOvershootShares * 4, config.repairMinQty);
  if (shareGap > fixedMax + 1e-9 && shareGap <= fixedMax + overfillCloseAllowance + 1e-9) {
    return normalizeOrderSize(shareGap, config.repairMinQty);
  }
  return fixedMax;
}
...
function aggressiveLateHighLowStagedRecycleAllowed(
  config: XuanStrategyConfig,
  state: XuanMarketState,
  candidateContext: FreshCycleCandidateContext | undefined,
): boolean {
  const effectivePair = candidateContext?.effectivePair;
  const rawPair = candidateContext?.rawPair;
  const highSidePrice = candidateContext?.highSidePrice;
  const lowSidePrice = candidateContext?.lowSidePrice;
  const seedPrice = candidateContext?.seedPrice;
  if (
    !candidateContext ||
    effectivePair === undefined ||
    rawPair === undefined ||
    highSidePrice === undefined ||
    lowSidePrice === undefined ||
    seedPrice === undefined ||
    !Number.isFinite(effectivePair) ||
    !Number.isFinite(rawPair) ||
    !Number.isFinite(highSidePrice) ||
    !Number.isFinite(lowSidePrice) ||
  ...
  if (
    !isAggressivePublicFootprint(config) ||
    !state.fillHistory.some((fill) => fill.side === "BUY") ||
    candidateContext.ctx.secsFromOpen < 135 ||
    candidateContext.ctx.secsToClose <= config.finalWindowCompletionOnlySec ||
    Math.abs(state.upShares - state.downShares) > xuanFreshCycleFlatDustThreshold(config, state) + 1e-9
  ) {
    return false;
  }
  const largeLotFootprint = hasAggressiveLargeLotFootprint(config, state);
  const highEnough = largeLotFootprint
    ? highSidePrice >= Math.max(0.93, config.highSidePriceThreshold + 0.04) - 1e-9
    : highSidePrice >= 0.88 - 1e-9;
  const cheapOppositeVisible = largeLotFootprint
    ? lowSidePrice <= Math.min(0.08, config.lowSideMaxForHighCompletion) + 1e-9
    : lowSidePrice <= 0.15 + 1e-9;
  const rawPairCap = largeLotFootprint ? 1.02 : 1.03;
  const seedIsHighSide = seedPrice >= highSidePrice - 1e-9;
  return seedIsHighSide && highEnough && cheapOppositeVisible && rawPair <= rawPairCap + 1e-9;
}

function hasAggressiveLargeLotFootprint(config: XuanStrategyConfig, state: XuanMarketState): boolean {
  const largeLot = config.liveSmallLotLadder[3] ?? config.liveSmallLotLadder.at(-1) ?? config.defaultLot;
  return state.fillHistory.some((fill) => fill.side === "BUY" && fill.size >= largeLot * 0.9 - 1e-9);
}

function aggressiveFamilyHighLowContinuationAllowed(
  config: XuanStrategyConfig,
  state: XuanMarketState,
  candidateContext: FreshCycleCandidateContext | undefined,
): boolean {
  ...
  const baseLot = Math.max(config.liveSmallLotLadder[0] ?? config.defaultLot, state.market.minOrderSize);
  const seedIsHighSide = seedPrice >= highSidePrice - 1e-9;
  const midCampaignContinuation =
    state.mergeHistory.length === 0 &&
    mergeableShares(state) >= baseLot * 2 - 1e-9 &&
    candidateContext.ctx.secsFromOpen >= 146 &&
    candidateContext.ctx.secsFromOpen <= 155 &&
    seedIsHighSide &&
    highSidePrice >= 0.58 - 1e-9 &&
    highSidePrice <= 0.82 + 1e-9 &&
    lowSidePrice >= 0.18 - 1e-9 &&
  ...
  const postMergeRecycleContinuation =
    ...
    seedIsHighSide &&
    highSidePrice >= 0.5 - 1e-9 &&
    highSidePrice <= 0.7 + 1e-9 &&
    lowSidePrice >= 0.3 - 1e-9 &&
    lowSidePrice <= 0.55 + 1e-9;
  return midCampaignContinuation || postMergeRecycleContinuation;
}
...
function newCyclePacingSkipReason(
  ...
  const aggressiveFamilyCycleReleaseActive =
    ...
    (
      ...
      (ctx.secsFromOpen >= 130 && matchedQty >= Math.max(state.market.minOrderSize, config.completionMinQty) - 1e-9)
    );
  if (
...
function freshCycleCandidateSkipReason(
  ...
  if (fixedFiveDirectionalSeedAllowed(config, state, candidateContext)) {
    return undefined;
  }
  if (
    isAggressivePublicFootprint(config) &&
    ...
    candidateContext.seedPrice !== undefined &&
    candidateContext.oppositePrice !== undefined &&
    candidateContext.seedPrice >= candidateContext.oppositePrice - 1e-9 &&
    candidateContext.rawPair <= 1.02 + 1e-9 &&
    candidateContext.effectivePair <= 1.06 + 1e-9 &&
    candidateContext.highSidePrice >= 0.5 - 1e-9 &&
    candidateContext.highSidePrice <= 0.7 + 1e-9 &&
    candidateContext.lowSidePrice >= 0.3 - 1e-9 &&
    candidateContext.lowSidePrice <= 0.55 + 1e-9
  ) {
    return undefined;
  }
  if (aggressiveLateHighLowStagedRecycleAllowed(config, state, candidateContext)) {
    return undefined;
  }
  if (aggressiveFamilyHighLowContinuationAllowed(config, state, candidateContext)) {
    return undefined;
  }
...
export function evaluateEntryBuys(
  ...
  const aggressiveOpeningDepthWait =
    config.botMode === "XUAN" &&
    config.xuanCloneMode === "PUBLIC_FOOTPRINT" &&
    config.xuanCloneIntensity === "AGGRESSIVE" &&
    totalShares <= Math.max(config.postMergeFlatDustShares, 1e-6) &&
    ...
    ctx.secsFromOpen < 12 &&
    Math.min(
      books.depthAtOrBetter("UP", Math.min(1, books.bestAsk("UP") + Math.max(books.tickSize(), 0.01)), "ask"),
      books.depthAtOrBetter("DOWN", Math.min(1, books.bestAsk("DOWN") + Math.max(books.tickSize(), 0.01)), "ask"),
    ) < Math.max(150, config.liveSmallLotLadder[0] ?? config.defaultLot);
  if (aggressiveOpeningDepthWait) {
    return {
      decisions: [],
      trace: {
        mode: "balanced_pair",
        ...
        shareGap,
        pairCap,
        skipReason: "xuan_open_liquidity_wait",
        recentBadCycleCount: freshCycleStats.recentBadCycleCount,
        candidates: [],
      },
    };
  ...
  if (useBalancedPairPath) {
    ...
    const fixedFivePairCycleOnly =
      fixedFiveEntryClipMax(config) !== undefined &&
      shareGap <= xuanFreshCycleFlatDustThreshold(config, state) + 1e-9;
    const fixedFiveDirectionalTemporalSeed =
      fixedFivePairCycleOnly &&
      temporalSeedEvaluation.decision !== undefined &&
      temporalSeedEvaluation.trace.some(
        (seedTrace) =>
          seedTrace.allowed &&
          temporalSeedEvaluation.decision !== undefined &&
          fixedFiveDirectionalTemporalSeedTraceAllowed({
            config,
            state,
            ctx,
            decision: temporalSeedEvaluation.decision,
            seedTrace,
          }),
      );
    ...
    if (stagedDebtReducingFlow && stagedDebtReducingCandidate) {
      return {
        ...
        trace: {
          ...
          plannedOppositeMissingQty: stagedDebtReducingFlow.plannedOppositeQty,
          plannedPairGroupOpenedAt: state.market.startTs + ctx.secsFromOpen,
          plannedLowSideAvg: stagedDebtReducingFlow.decision.expectedAveragePrice,
          plannedOppositeMaxPrice: stagedDebtReducingFlow.plannedOppositeMaxPrice,
          plannedOppositeMinWaitSec: stagedDebtReducingFlow.plannedOppositeMinWaitSec,
          plannedOppositeDeadlineSec: stagedDebtReducingFlow.plannedOppositeDeadlineSec,
    ...
    if (inspected.bestCandidate && !preferTemporalCloneCycle && !fixedFivePairCycleOnly) {
    ...
    if (temporalSeedEvaluation.decision && (!fixedFivePairCycleOnly || fixedFiveDirectionalTemporalSeed)) {
      ...
      return {
        ...
        trace: {
          ...
          ...(fixedFiveDirectionalTemporalSeed ? { fairValueFallbackReason: "fixed_five_directional_chainlink_seed" } : {}),
          ...(fixedFiveDirectionalTemporalSeed
            ? {}
            : {
                skipReason: preferTemporalCloneCycle
    ...
    if (inspected.bestCandidate && !fixedFivePairCycleOnly) {
      const pairPlan = buildBalancedPairEntryPlan(
        config,
        state,
        ctx.secsFromOpen,
    ...
    if (fixedFivePairCycleOnly) {
      ...
      return {
        ...
        trace: {
          ...trace,
          seedCandidates: temporalSeedEvaluation.trace,
          skipReason: `${pairSkipReason}+xuan_fixed_five_pair_cycle_only`,
          cycleSkippedReason: "xuan_fixed_five_pair_cycle_only",
        },
      };
    }
    ...
    const allFreshCycleTraces = [...seedEvaluation.trace, ...temporalSeedEvaluation.trace, ...inspected.traces];
    const xuanStrictCycleSkippedReason = allFreshCycleTraces.find(
      (trace) => trace.cycleSkippedReason === "xuan_strict_micro_reentry_disabled",
    )?.cycleSkippedReason;
    const marketBasketContinuationCycleSkippedReason =
      allFreshCycleTraces.find((trace) => {
    ...
    const pairSkipReason = determineBalancedPairSkipReason(inspected.maxCandidateSize, inspected.traces);
    const explicitSingleLegSkipReason =
      cycleSkippedReason?.startsWith("xuan_") ||
      cycleSkippedReason?.startsWith("avg_improving_") ||
      cycleSkippedReason?.startsWith("flow_shaping_") ||
      cycleSkippedReason === "high_low_effective_not_debt_reducing" ||
      cycleSkippedReason === "market_basket_continuation_rejected"
        ? cycleSkippedReason
        : undefined;
  ...
  const aggressivePublicFootprint =
    config.xuanCloneMode === "PUBLIC_FOOTPRINT" && config.xuanCloneIntensity === "AGGRESSIVE";
  const materialOpenPlannedOpposite = materialOpenPlannedOppositeState(
    config,
    state,
    state.market.startTs + ctx.secsFromOpen,
  );
  ...
  const buildOverlapPairReentry = (): EntryEvaluation | undefined => {
    ...
    if (materialOpenPlannedOpposite !== undefined) {
      return undefined;
    }
    const pairPlan = buildBalancedPairEntryPlan(
      config,
  ...
  const buildOverlapTemporalSeedEvaluation = (): EntryEvaluation | undefined =>
    materialOpenPlannedOpposite === undefined && overlapTemporalSeed?.decision
      ? {
  ...
  const plannedOppositeDutyActive =
    ...
    (aggressivePublicFootprint ||
      ...
      exactCompletionQtyPrior !== undefined) &&
    (aggressivePublicFootprint || plannedOppositeTimingReady);
  ...
  const campaignPhaseMaxQty =
    ...
    !activeCompletionQtyPrior
      ...
      : phaseMaxQty;
  const fixedFiveResidualRepairMax = fixedFiveResidualRepairClipMax(config, shareGap);
  const effectiveCampaignPhaseMaxQty =
    fixedFiveResidualRepairMax !== undefined && Number.isFinite(campaignPhaseMaxQty)
      ? Math.max(campaignPhaseMaxQty, fixedFiveResidualRepairMax)
      : campaignPhaseMaxQty;
  const phasedRepairSize = normalizeOrderSize(
    Math.min(repairSize, Number.isFinite(effectiveCampaignPhaseMaxQty) ? effectiveCampaignPhaseMaxQty : repairSize),
    config.repairMinQty,
  );
  ...
  const plannedOppositeQty =
    plannedOppositeDutyActive && plannedOpposite
      ...
      : 0;
  const repairCandidateSizes = buildXuanPlannedOppositeRepairCandidateSizes({
    config,
    plannedOppositeQty,
    shareGap,
  ...
  for (const repairCandidateSize of repairCandidateSizes) {
    ...
    const minExecutableRepairQty =
      aggressivePublicFootprint && plannedOppositeDutyActive
        ? Math.max(state.market.minOrderSize, config.repairMinQty)
        : config.repairMinQty;
    const executableSize = normalizeOrderSize(execution.filledSize, minExecutableRepairQty);
    if (executableSize <= 0) {
      const openPlannedOppositeNoCloseablePath =
        aggressivePublicFootprint &&
        plannedOppositeDutyActive &&
        plannedOpposite !== undefined &&
        plannedOpposite.plannedOppositeAgeSec >= plannedOppositeDeadlineAgeSec(config) - 1e-9;
      const plannedOppositeSoftWait =
        aggressivePublicFootprint &&
        plannedOppositeDutyActive &&
        plannedOpposite !== undefined &&
        plannedOpposite.plannedOppositeAgeSec < plannedOppositeMinWaitSec(config, ctx.secsFromOpen, ctx.secsToClose) - 1e-9;
      const repairDepthSkipReason = openPlannedOppositeNoCloseablePath
        ? "xuan_open_planned_opposite_no_closeable_path"
        : plannedOppositeSoftWait
          ? "xuan_planned_opposite_wait"
          : "lagging_depth";
      lastBlockedRepairEvaluation = {
        ...
        trace: {
          ...
          continuationRejectedReason: repairDepthSkipReason,
          skipReason: repairDepthSkipReason,
        },
      };
      continue;
    }
    ...
    const xuanResidualDutyMaxQty = Math.max(
      ...
      xuanFamilyResidualDutyActive ? Math.min(config.xuanBasketCampaignCompletionClipMaxQty, oldGap * 1.15) : 0,
	    ...
	    const nonExactFamilyResidualOvershootBlocked =
	      ...
	      newGap > Math.max(config.postMergeFlatDustShares, config.maxCompletionOvershootShares) + 1e-9;
	    if (nonExactFamilyResidualOvershootBlocked) {
	      lastBlockedRepairEvaluation = {
	        ...
	        trace: {
	          ...
	          continuationRejectedReason: "xuan_residual_overshoot_wait",
	          overlapRepairOutcome: "wait",
	          skipReason: "xuan_residual_overshoot_wait",
	        },
	      };
	      continue;
	    }
    ...
    const openingCampaignDeadlineCompletionAllowed =
      aggressivePublicFootprint &&
      plannedOppositeCandidate &&
      plannedOppositeDeadlineActive &&
      plannedOppositeAgeSec >= 80 - 1e-9 &&
      ctx.secsFromOpen >= 120 &&
      ctx.secsFromOpen <= 145 &&
      repairCost <= 1.08 + 1e-9;
    ...
    const plannedOppositePriceTargetMet =
      !aggressivePublicFootprint ||
      Boolean(exactCompletionQtyPrior) ||
      ctx.secsToClose <= config.finalWindowCompletionOnlySec ||
    ...
    if (aggressiveOppositeHold && (aggressiveOppositeHold.force || !plannedOppositeProtectiveRelease)) {
      lastBlockedRepairEvaluation = {
        decisions: [],
        trace: {
          ...trace,
    ...
    if (
      ...
      lastBlockedRepairEvaluation = {
        ...
        trace: {
          ...
          plannedOppositeMaxPrice: normalizeTraceNumber(plannedOppositeMaxPrice),
          continuationRejectedReason: plannedOppositeDeadlineActive
            ? "xuan_open_planned_opposite_no_closeable_path"
            : "xuan_pair_cost_wait",
          overlapRepairOutcome: "wait",
          skipReason: plannedOppositeDeadlineActive
            ? "xuan_open_planned_opposite_no_closeable_path"
            : "xuan_pair_cost_wait",
        },
      };
    ...
    const strictXuanCompletionReleaseAllowed =
      ...
      (executableSize <= oldGap + config.maxCompletionOvershootShares + 1e-9 ||
        xuanRoleSequenceOvershootAllowed) &&
      (xuanCostDisciplinedCompletion || plannedOppositeCloseableRelease);
    ...
    const detailedTrace: EntryDecisionTrace = {
      ...
      ...(plannedOppositeCompletionAllowed && plannedOpposite
        ? {
            ...
            plannedOppositeAgeSec: plannedOpposite.plannedOppositeAgeSec,
            plannedOppositeMaxPrice: normalizeTraceNumber(plannedOppositeMaxPrice),
            plannedOppositeMinWaitSec: plannedOppositeMinWaitSec(config, ctx.secsFromOpen, ctx.secsToClose),
            plannedOppositeDeadlineSec: normalizeTraceNumber(
              (plannedOpposite.plannedPairGroupOpenedAt - state.market.startTs) +
                plannedOppositeDeadlineAgeSec(config),
            ),
    ...
    const repairScore =
      plannedOppositeCompletionAllowed || strictXuanCompletionReleaseAllowed
        ? -executableSize
        : completionCandidateRankScore(config, {
            costWithFees: repairCost,
            missingSidePrice: execution.averagePrice,
            candidateSize: executableSize,
...
function buildXuanPlannedOppositeRepairCandidateSizes(args: {
  config: XuanStrategyConfig;
  plannedOppositeQty: number;
  shareGap: number;
  baseRepairCandidateSizes: number[];
}): number[] {
  return buildXuanStrictPlannedOppositeCandidateSizes({
    config: args.config,
    plannedOppositeQty: args.plannedOppositeQty,
    missingQty: args.shareGap,
    minOrderSize: args.config.repairMinQty,
    baseCandidateSizes: args.baseRepairCandidateSizes,
    normalize: (size) => normalizeOrderSize(size, args.config.repairMinQty),
  });
}
...
function evaluateTemporalSingleLegSeed(
  ...
  const exactSeedPriorQty =
    aggressivePublicFootprint && seedSequencePrior?.scope === "exact"
      ...
      : undefined;
  const lateHighLowStagedLot =
    aggressivePublicFootprint &&
    state.fillHistory.some((fill) => fill.side === "BUY") &&
    hasAggressiveLargeLotFootprint(config, state) &&
    ctx.secsFromOpen >= 165 &&
    ctx.secsToClose > config.finalWindowCompletionOnlySec &&
    Math.abs(state.upShares - state.downShares) <= xuanFreshCycleFlatDustThreshold(config, state) + 1e-9
      ? Math.max(config.defaultLot, ...config.liveSmallLotLadder)
      : undefined;
  ...
  const requestedSeedLot =
    exactSeedPriorQty !== undefined
      ...
      : lateHighLowStagedLot !== undefined
        ? Math.max(ctx.lot, lateHighLowStagedLot)
      : ctx.lot;
  ...
  const freshMidOpeningHighSide =
    aggressivePublicFootprint &&
    state.fillHistory.length === 0 &&
    state.mergeHistory.length === 0 &&
    ctx.secsFromOpen >= 10 &&
    Math.max(books.bestAsk("UP"), books.bestAsk("DOWN")) <= 0.56 + 1e-9 &&
    Math.min(books.bestAsk("UP"), books.bestAsk("DOWN")) >= 0.44 - 1e-9
      ? books.bestAsk("UP") >= books.bestAsk("DOWN")
        ? "UP"
        : "DOWN"
      : undefined;
  ...
  const priorityScore = (candidate: {
    ...
    Number(
      (
        ...
        (freshMidOpeningHighSide === candidate.side ? 20 : 0) +
  ...
  const buildTemporalSeedCandidate = (
    ...
    const finalResidualDutyActive =
      ctx.secsFromOpen >= 250 &&
      oldGap >= Math.max(state.market.minOrderSize, config.repairMinQty) - 1e-9;
    const strictResidualCompletionCostBlockReason =
      completingExistingResidual
        ? strictXuanResidualCompletionCostBlockReason({
            config,
            currentEffectivePair: matchedEffectivePair,
            candidateEffectivePair: referencePairCost,
    ...
    const protectedCampaignResidualCompletion =
      completingExistingResidual &&
      strictResidualCompletionCostBlockReason === undefined &&
    ...
    const temporalSeedCostPlan = stagedPairCostPlan({
      config,
      seedPrice: seedQuote.averagePrice,
      oppositePrice: oppositeQuote.averagePrice,
      currentEffectivePair: matchedEffectivePair,
    });
    const strictSeedCostBlockReason = completingExistingResidual
      ? undefined
      : strictXuanSeedCostBlockReason({
          config,
          state,
          secsFromOpen: ctx.secsFromOpen,
          referencePairCost,
    ...
    const freshCycleCandidateContext: FreshCycleCandidateContext = {
      ...
      seedPrice: seedQuote.averagePrice,
      oppositePrice: referenceOppositePrice,
    };
    const postMergeReentryCompletionBlockReason = fixedFivePostMergeReentryCompletionBlockReason({
      config,
      state,
      side,
      seedSize: executableSize > 0 ? executableSize : requestedSize,
    });
    const aggressiveLateHighLowStagedEntry =
      aggressiveLateHighLowStagedRecycleAllowed(config, state, freshCycleCandidateContext);
    let skipReason: string | undefined;

    if (denseCycleThrottle && !aggressiveLateHighLowStagedEntry) {
    ...
    if (skipReason === undefined) {
      if (materialOpenPlannedOpposite) {
        skipReason = "xuan_material_planned_opposite_open";
      } else if (postMergeReentryCompletionBlockReason) {
        skipReason = postMergeReentryCompletionBlockReason;
      } else if (strictNormalMicroSeedBlocked) {
        skipReason = "xuan_strict_seed_abort_micro_reentry";
      } else if (strictResidualCompletionCostBlockReason) {
        skipReason = strictResidualCompletionCostBlockReason;
      } else if (strictSeedCostBlockReason && !aggressiveLateHighLowStagedEntry) {
        skipReason = strictSeedCostBlockReason;
  ...
  const freshMidOpeningCandidateSide =
    aggressivePublicFootprint &&
    state.fillHistory.length === 0 &&
    state.mergeHistory.length === 0 &&
    ctx.secsFromOpen >= 10
      ? (() => {
          const up = standardCandidates.find((candidate) => candidate.side === "UP");
          const down = standardCandidates.find((candidate) => candidate.side === "DOWN");
          if (!up || !down) {
            return undefined;
          }
          const high = Math.max(up.seedQuote.averagePrice, down.seedQuote.averagePrice);
          const low = Math.min(up.seedQuote.averagePrice, down.seedQuote.averagePrice);
          return high <= 0.56 + 1e-9 && low >= 0.44 - 1e-9
            ? up.seedQuote.averagePrice >= down.seedQuote.averagePrice
              ? "UP"
              : "DOWN"
            : undefined;
        })()
  ...
  if (freshMidOpeningCandidateSide) {
    const preferredIndex = candidates.findIndex(
      (candidate) =>
        candidate.side === freshMidOpeningCandidateSide &&
        candidate.sizingMode === "standard" &&
...
function inspectBalancedPairCandidates(
  ...
  const basketState = marketBasketStateTrace(config, state);
  const fixedFiveMaxEntrySize = fixedFiveEntryClipMax(config);
  const baseMaxCandidateSize = normalizeOrderSize(
    Math.min(
      requestedMaxLot,
      fixedFiveMaxEntrySize ?? Number.POSITIVE_INFINITY,
      Math.max(0, config.maxMarketSharesPerSide - state.upShares),
      Math.max(0, config.maxMarketSharesPerSide - state.downShares),
  ...
  for (const requestedSize of requestedSizes) {
    ...
    const currentBasketEffectiveForCostDiscipline =
      mergeableShares(state) > 1e-6 ? basketState.basketEffectiveAvg : Number.POSITIVE_INFINITY;
    const xuanPairCostDisciplined = xuanPairCostImprovesOrMeetsTarget(
      config,
      currentBasketEffectiveForCostDiscipline,
      pairCost,
    );
    const stagedSequenceSeedSide = aggressivePublicFootprint
      ? preferredStagedPairSeedSideFromPrices(
          config,
          state,
          secsFromOpen,
          upExecution.averagePrice,
      ...
      : undefined;
    const stagedSequenceSeedPrice =
      stagedSequenceSeedSide === "UP"
        ? upExecution.averagePrice
        : stagedSequenceSeedSide === "DOWN"
          ? downExecution.averagePrice
          : undefined;
    const stagedSequenceOppositePrice =
      stagedSequenceSeedSide === "UP"
        ? downExecution.averagePrice
        : stagedSequenceSeedSide === "DOWN"
          ? upExecution.averagePrice
          : undefined;
    const stagedSequenceSeedIsLow =
      stagedSequenceSeedPrice !== undefined &&
      stagedSequenceOppositePrice !== undefined &&
      stagedSequenceSeedPrice <= stagedSequenceOppositePrice + 1e-9;
    const stagedSequenceCostPlan =
      stagedSequenceSeedPrice !== undefined && stagedSequenceOppositePrice !== undefined
        ? stagedPairCostPlan({
            config,
            seedPrice: stagedSequenceSeedPrice,
            oppositePrice: stagedSequenceOppositePrice,
            currentEffectivePair: currentBasketEffectiveForCostDiscipline,
          })
        : undefined;
    const xuanStrictSeedCostReason =
      stagedSequenceCostPlan !== undefined && stagedSequenceSeedPrice !== undefined
        ? strictXuanSeedCostBlockReason({
            config,
            state,
            secsFromOpen,
    ...
    const xuanStagedPairCostReducingSetup =
      stagedSequenceCostPlan?.viable === true &&
      (stagedSequenceSeedIsLow || xuanPairCostDisciplined);
    const postMergeFlatImmediatePairRecycle =
      isPostMergeFlatImmediatePairRecycle({
        config,
        state,
        referencePairCost: pairCost,
        requestedSize,
    ...
    const balancedMicroPairReady =
      ...
      !staleCheapOppositeQuote;
    const xuanStrictMicroPairBlocked =
      aggressivePublicFootprint &&
      config.xuanMicroPairContinuationEnabled &&
      config.botMode === "XUAN" &&
      config.xuanCloneMode === "PUBLIC_FOOTPRINT" &&
    ...
    const aggressiveAvgImprovingShouldStage =
      ...
      pairCost > strictXuanPairCostTargetCap(config) + 1e-9;
    const xuanStrictSequenceContinuation =
      ...
      (xuanPairCostDisciplined || xuanStagedPairCostReducingSetup || postMergeFlatImmediatePairRecycle) &&
      !xuanStrictSeedCostBlocked &&
      !staleCheapOppositeQuote;
    const xuanBalancedPairContinuationOverride =
      ...
      !staleCheapOppositeQuote &&
      !xuanStrictSeedCostBlocked &&
      !aggressiveAvgImprovingShouldStage &&
      (
        xuanStrictSequenceContinuation ||
        xuanMicroPairContinuation ||
        xuanPairCostDisciplined ||
        (
          basketProjection !== undefined &&
          basketProjection.improvement >= config.marketBasketMinAvgImprovement - 1e-9 &&
    ...
    const verdict =
      ...
      verdict === "pair_cap"
        ? strictHighLowHardSweepBlocked
          ...
          : xuanStrictSeedCostReason
            ? xuanStrictSeedCostReason
          : xuanStrictMicroPairBlocked
            ? "xuan_strict_micro_reentry_disabled"
          : effectiveFreshCycleSkipReason
            ? effectiveFreshCycleSkipReason
          : staleCheapOppositeQuote
    ...
    traces.push({
      ...
      ...(xuanMicroPairProjectedTooExpensive
        ...
        : xuanStrictMicroPairBlocked
          ? { cycleSkippedReason: "xuan_strict_micro_reentry_disabled" }
...
function evaluateSingleLegSeed(
  ...
  for (const side of sideOrder) {
    ...
    const aggressiveLateHighLowStagedEntry =
      aggressiveLateHighLowStagedRecycleAllowed(config, state, freshCycleCandidateContext) &&
      seedQuote.averagePrice >= oppositeQuote.averagePrice - 1e-9;
    ...
    const postMergeHighSideSetup =
      ...
      seedQuote.averagePrice >= oppositeQuote.averagePrice - 1e-9 &&
      seedQuote.averagePrice >= 0.5 - 1e-9 &&
      seedQuote.averagePrice <= 0.7 + 1e-9 &&
      rawPairCost <= 1.02 + 1e-9 &&
      referencePairCost <= 1.06 + 1e-9 &&
      pairExecutableSize > 0;
    if (
      (postProfitLowSideSetup || postMergeHighSideSetup) &&
      (
    ...
    const currentBasketEffectiveForSeed =
      basketState.mergeableQty > 1e-6 ? basketState.basketEffectiveAvg : Number.POSITIVE_INFINITY;
    const seedPairCostDisciplined = xuanPairCostImprovesOrMeetsTarget(
      config,
      currentBasketEffectiveForSeed,
      referencePairCost,
    );
    const seedCostPlan = stagedPairCostPlan({
      config,
      seedPrice: seedQuote.averagePrice,
      oppositePrice: oppositeQuote.averagePrice,
      currentEffectivePair: currentBasketEffectiveForSeed,
    });
    const postMergeReentryCompletionBlockReason = fixedFivePostMergeReentryCompletionBlockReason({
      config,
      state,
      side,
      seedSize: pairExecutableSize > 0 ? pairExecutableSize : candidateSize,
    });
    const seedIsLowSide = seedQuote.averagePrice <= oppositeQuote.averagePrice + 1e-9;
    const highSideCloseableTargetAllowed =
      !seedIsLowSide &&
      seedCostPlan.viable &&
    ...
    if (skipReason === undefined) {
      if (
        ...
        !config.coveredSeedAllowSamePairgroupOppositeOrder &&
        !canUseInventoryCover &&
        !config.allowNakedSingleLegSeed
      ) {
        ...
        skipReason = "xuan_pair_cost_wait";
      } else if (!postMergeHighSideSetup && !config.allowNakedSingleLegSeed && !canUseInventoryCover && !requiresSamePairgroupOppositeOrder) {
    ...
    const useStagedPostProfitLowSideEntry =
      ...
      requiresSamePairgroupOppositeOrder &&
      !canUseInventoryCover;
    const useStagedPostMergeHighSideEntry =
      postMergeHighSideSetup &&
      !canUseInventoryCover;
    ...
    const fairValueFallbackReason = shouldAllowCoveredSeedMissingFairValue({
      ...
      : postMergeHighSideSetup && baseFairValueDecision.reason === "fair_value_missing"
        ? "post_merge_high_side_setup"
      : undefined;
    if (
      skipReason === undefined &&
    ...
    traces.push({
      ...
      ...(postMergeHighSideSetup ? { fairValueFallbackReason: "post_merge_high_side_setup" } : {}),
      ...(useStagedBorderlineEntry
        ? {
            stagedEntry: true,
      ...
      ...(useStagedPostProfitLowSideEntry
        ...
        : {}),
      ...(useStagedPostMergeHighSideEntry
        ? {
            stagedEntry: true,
            plannedOppositeSide: oppositeSide,
            plannedOppositeQty: pairExecutableSize,
    ...
    if (!decisions && skipReason === undefined && fairValueDecision.allowed && orphanRisk.allowed) {
      ...
      decisions = useStagedBorderlineEntry || useStagedPostProfitLowSideEntry || useStagedPostMergeHighSideEntry
        ? [seedDecision]
...
function determineBalancedPairSkipReason(
  ...
  if (gateReasons.has("xuan_strict_micro_reentry_disabled")) return "xuan_strict_micro_reentry_disabled";
...
function strictXuanSeedCostBlockReason(args: {
  ...
  if (!isAggressivePublicFootprint(args.config) || !Number.isFinite(args.referencePairCost)) {
    return undefined;
  }

  const exploratoryMaxQty = Math.max(20, args.state.market.minOrderSize);
  const largeSeed = args.requestedSize > exploratoryMaxQty + 1e-9;
  const closeableCap = strictXuanCloseablePairCostCap(args.config);
  const pendingMatchedQty = mergeableShares(args.state);
  const postMergeFlatImmediatePairRecycle = isPostMergeFlatImmediatePairRecycle({
    config: args.config,
    state: args.state,
    referencePairCost: args.referencePairCost,
    requestedSize: args.requestedSize,
    secsFromOpen: args.secsFromOpen,
    seedRoute: args.seedRoute,
    hasVisibleOppositePath: args.hasVisibleOppositePath,
  });
  const targetEffectivePair = args.targetEffectivePair ?? args.referencePairCost;
  const spread =
    args.seedPrice !== undefined && args.oppositePrice !== undefined
      ? Math.abs(args.seedPrice - args.oppositePrice)
      : 0;
  const freshMarket = args.state.fillHistory.length === 0 && args.state.mergeHistory.length === 0;
  const projectedCloseablePath =
    args.hasVisibleOppositePath &&
    Number.isFinite(targetEffectivePair) &&
    targetEffectivePair <= closeableCap + 1e-9;
  const immediatelyCloseablePath =
    args.hasVisibleOppositePath &&
    args.referencePairCost <= closeableCap + 1e-9;
  const highSideProjectedCloseablePath = args.lowSideSeed === false && projectedCloseablePath;
  const largeStagedLowSideTargetAllowed =
    largeSeed &&
    args.lowSideSeed === true &&
    args.hasVisibleOppositePath &&
    Number.isFinite(targetEffectivePair) &&
    targetEffectivePair <= strictXuanPairCostTargetCap(args.config) + 1e-9;
  const freshMidHighSideOpeningAllowed =
    largeSeed &&
    args.lowSideSeed === false &&
    freshMarket &&
    args.secsFromOpen >= 0 &&
    args.secsFromOpen < 25 &&
    args.requestedSize <= 45 + 1e-9 &&
    (
      spread >= 0.18 - 1e-9 ||
      (
        spread <= 0.08 + 1e-9 &&
        args.seedPrice !== undefined &&
        args.oppositePrice !== undefined &&
        Math.max(args.seedPrice, args.oppositePrice) <= 0.54 + 1e-9 &&
        Math.min(args.seedPrice, args.oppositePrice) >= 0.47 - 1e-9
      )
    ) &&
    args.referencePairCost <= 1.055 + 1e-9;
  const matureHighSideMicroProbeAllowed =
    largeSeed &&
    args.lowSideSeed === false &&
    freshMarket &&
  ...
  const freshLargeHighSideOpeningAllowed =
    ...
    freshMarket &&
    args.secsFromOpen >= 25 &&
    args.secsFromOpen < 65 &&
    args.requestedSize >= 110 - 1e-9 &&
    args.requestedSize <= 150 + 1e-9 &&
    spread >= 0.18 - 1e-9 &&
    args.seedPrice !== undefined &&
    args.seedPrice >= 0.58 - 1e-9 &&
    args.seedPrice <= 0.7 + 1e-9 &&
    args.referencePairCost <= 1.06 + 1e-9;
  const postProfitHighSideContinuationAllowed =
    largeSeed &&
    args.lowSideSeed === false &&
    args.hasVisibleOppositePath &&
    args.state.mergeHistory.length === 0 &&
    pendingMatchedQty >= 180 - 1e-9 &&
    args.secsFromOpen >= 35 &&
    args.secsFromOpen < 90 &&
    args.seedPrice !== undefined &&
    args.seedPrice >= 0.58 - 1e-9 &&
    args.seedPrice <= 0.82 + 1e-9 &&
    args.referencePairCost <= 1.06 + 1e-9 &&
    matchedEffectivePairCost(args.state, args.config.cryptoTakerFeeRate) <=
      args.config.marketBasketMergeEffectivePairCap + 1e-9;
  const familyHighSideContinuationAllowed =
    largeSeed &&
    args.lowSideSeed === false &&
    args.hasVisibleOppositePath &&
    pendingMatchedQty >=
      Math.max(args.config.liveSmallLotLadder[0] ?? args.config.defaultLot, args.state.market.minOrderSize) * 2 - 1e-9 &&
    args.secsFromOpen >= 146 &&
    args.secsFromOpen <= 155 &&
    args.seedPrice !== undefined &&
    args.seedPrice >= 0.58 - 1e-9 &&
    args.seedPrice <= 0.82 + 1e-9 &&
    args.referencePairCost <= 1.06 + 1e-9 &&
    matchedEffectivePairCost(args.state, args.config.cryptoTakerFeeRate) <=
      args.config.marketBasketMergeEffectivePairCap + 1e-9;
  const postMergeHighSideRecycleAllowed =
    largeSeed &&
    args.lowSideSeed === false &&
    args.hasVisibleOppositePath &&
    args.state.mergeHistory.length > 0 &&
    ...
    args.seedPrice !== undefined &&
    args.seedPrice >= 0.5 - 1e-9 &&
    args.seedPrice <= 0.7 + 1e-9 &&
    args.referencePairCost <= 1.06 + 1e-9;

  const tightMidEarlyPair =
    spread <= 0.08 + 1e-9 &&
    args.seedPrice !== undefined &&
    args.oppositePrice !== undefined &&
    Math.max(args.seedPrice, args.oppositePrice) <= 0.54 + 1e-9 &&
  ...
  if (
    ...
    return "xuan_early_balanced_regime_wait";
  }
  if (
    freshMarket &&
    !args.exactPriorActive &&
    ...
    args.lowSideSeed === true &&
    spread >= 0.18 - 1e-9 &&
    args.requestedSize >= 20 - 1e-9
  ) {
    return "xuan_high_side_first_repair_path";
  }
  const recycleBasketReady =
    pendingMatchedQty >=
      Math.max(
        args.config.marketBasketMinMergeShares,
        (args.config.liveSmallLotLadder[0] ?? args.config.defaultLot) * 2,
      ) -
        1e-9;
  if (
    largeSeed &&
  ...
  if (
    largeSeed &&
    !args.exactPriorActive &&
    !projectedCloseablePath &&
    !matureHighSideMicroProbeAllowed &&
    !freshLargeHighSideOpeningAllowed &&
    !familyHighSideContinuationAllowed &&
    !postMergeHighSideRecycleAllowed
  ) {
    return "xuan_open_planned_opposite_no_closeable_path";
  }
  if (
    largeSeed &&
    pendingMatchedQty >= args.state.market.minOrderSize - 1e-9 &&
    !args.exactPriorActive &&
    !highSideProjectedCloseablePath &&
    !matureHighSideMicroProbeAllowed &&
    !freshLargeHighSideOpeningAllowed &&
    !familyHighSideContinuationAllowed &&
    !postMergeHighSideRecycleAllowed &&
    (!Number.isFinite(args.referencePairCost) || args.referencePairCost > closeableCap + 1e-9)
  ) {
    return "xuan_seed_wait_for_recycle";
  }
  if (largeSeed && args.referencePairCost > 1.06 + 1e-9) {
    return "xuan_high_cost_seed";
  }
  if (!postMergeFlatImmediatePairRecycle && !plannedOppositeLateSeedDeadlineFits(args)) {
    return "xuan_late_seed_deadline_after_final_merge";
  }
  if (args.referencePairCost > 1.045 + 1e-9 && args.referencePairCost <= 1.06 + 1e-9) {
    if (
      largeSeed &&
      !largeStagedLowSideTargetAllowed &&
      !freshMidHighSideOpeningAllowed &&
      !matureHighSideMicroProbeAllowed &&
      !freshLargeHighSideOpeningAllowed &&
      !postProfitHighSideContinuationAllowed &&
      !familyHighSideContinuationAllowed &&
      !postMergeHighSideRecycleAllowed
    ) {
      return "xuan_high_cost_seed_exploratory_cap";
    }
    if (!args.hasVisibleOppositePath) {
      return "xuan_high_cost_seed_no_opposite_path";
    }
  }
  if (
    args.secsFromOpen >= 200 &&
    !args.exactPriorActive &&
    !postMergeFlatImmediatePairRecycle &&
    !highSideProjectedCloseablePath &&
    (!Number.isFinite(args.referencePairCost) || args.referencePairCost > closeableCap + 1e-9)
  ) {
    return "xuan_late_seed_pair_cost_wait";
  }
  if (
  ...
  if (
    ...
    return "xuan_post_merge_seed_pair_cost_wait";
  }
...
function strictXuanResidualCompletionCostBlockReason(args: {
  ...
  candidateEffectivePair: number;
  requestedSize: number;
  finalResidualDutyActive: boolean;
  exactPriorActive: boolean;
}): string | undefined {
  if (
    !isAggressivePublicFootprint(args.config) ||
    args.finalResidualDutyActive ||
    args.exactPriorActive ||
    !Number.isFinite(args.candidateEffectivePair)
  ) {
    return undefined;
  }

  const largeSeed = args.requestedSize > 20 + 1e-9;
  if (largeSeed && args.candidateEffectivePair > 1.06 + 1e-9) {
    return "xuan_high_cost_seed";
  }
  if (
    !xuanPairCostImprovesOrMeetsTarget(
      args.config,
      args.currentEffectivePair,
      args.candidateEffectivePair,
    )
  ) {
    return "xuan_pair_cost_wait";
  }
  return undefined;
}

function materialOpenPlannedOppositeState(
  config: XuanStrategyConfig,
  state: XuanMarketState,
  nowTs: number,
): ReturnType<typeof plannedOppositeCompletionState> {
  if (!isAggressivePublicFootprint(config)) {
    return undefined;
  }
  const plannedOpposite = plannedOppositeCompletionState(
  ...
  const materialThreshold = Math.max(
    state.market.minOrderSize,
    plannedOpposite.plannedOppositeQty * 0.15,
  );
  return plannedOpposite.plannedOppositeMissingQty >= materialThreshold - 1e-9
    ? plannedOpposite
    : undefined;
}
...
function preferredStagedPairSeedSideFromPrices(
  config: XuanStrategyConfig,
  state: XuanMarketState,
  secsFromOpen: number,
  upPrice: number,
  ...
  if (
    ...
    state.mergeHistory.length === 0 &&
    secsFromOpen >= 10 &&
    Math.max(upPrice, downPrice) <= 0.56 + 1e-9 &&
    Math.min(upPrice, downPrice) >= 0.44 - 1e-9
  ) {
  ...
  if (
    isAggressivePublicFootprint(config) &&
    state.mergeHistory.length > 0 &&
    state.mergeHistory.reduce((sum, merge) => sum + merge.amount, 0) >= 180 - 1e-9 &&
    secsFromOpen >= 210 &&
    Math.max(upPrice, downPrice) >= 0.55 - 1e-9 &&
    Math.max(upPrice, downPrice) <= 0.82 + 1e-9 &&
    Math.min(upPrice, downPrice) >= 0.18 - 1e-9
  ) {
    return upPrice >= downPrice ? "UP" : "DOWN";
  }
  ...
  if (
    ...
    state.mergeHistory.length > 0 &&
    secsFromOpen >= 160 &&
    Math.max(upPrice, downPrice) >= 0.85 - 1e-9 &&
    Math.min(upPrice, downPrice) <= 0.15 + 1e-9
  ) {
    return upPrice >= downPrice ? "UP" : "DOWN";
  }
...
function preferredStagedPairSeedSide(
  ...
  return preferredStagedPairSeedSideFromPrices(
    config,
    state,
    secsFromOpen,
...
function stagedPairCostPlan(args: {
  config: XuanStrategyConfig;
  seedPrice: number;
  oppositePrice: number;
  currentEffectivePair?: number | undefined;
}): {
  viable: boolean;
  targetMaxPrice: number;
  targetEffectivePair: number;
} {
  const seedEffectivePrice =
    args.seedPrice + takerFeePerShare(args.seedPrice, args.config.cryptoTakerFeeRate);
  const targetCap = strictXuanPairCostTargetCap(args.config);
  const targetMaxPrice = maxCompletionAskForCostCap(
    seedEffectivePrice,
    targetCap,
    args.config.cryptoTakerFeeRate,
  );
  const boundedTargetPrice = Math.min(args.oppositePrice, targetMaxPrice);
  const targetEffectivePair =
    targetMaxPrice > 0
      ? completionCost(seedEffectivePrice, boundedTargetPrice, args.config.cryptoTakerFeeRate)
      : Number.POSITIVE_INFINITY;
  return {
    viable:
      targetMaxPrice > 0 &&
      xuanPairCostImprovesOrMeetsTarget(
        args.config,
        args.currentEffectivePair ?? Number.POSITIVE_INFINITY,
        targetEffectivePair,
      ),
    targetMaxPrice: normalizeTraceNumber(targetMaxPrice),
    targetEffectivePair: Number.isFinite(targetEffectivePair)
      ? normalizeTraceNumber(targetEffectivePair)
      : Number.POSITIVE_INFINITY,
  };
}
...
function clipExecutionQuoteForStagedSeed(execution: ExecutionQuote, size: number): ExecutionQuote {
  return {
    requestedSize: size,
    filledSize: size,
    averagePrice: execution.averagePrice,
    limitPrice: execution.limitPrice,
    fullyFilled: execution.fullyFilled && execution.filledSize + 1e-9 >= size,
  };
}
...
function buildAggressiveStagedPairSeed(args: {
  ...
  const blockedPlan = (
    reason: string,
    extraTrace: Partial<EntryDecisionTrace> = {},
  ): BalancedPairEntryPlan => ({
    decisions: [],
    stateAfter: inventoryTraceState(args.state),
    tracePatch: {
      selectedMode: "PAIRGROUP_COVERED_SEED",
      stagedEntry: true,
      skipReason: reason,
      cycleSkippedReason: reason,
      ...extraTrace,
    },
  });
  const materialOpenPlannedOpposite = materialOpenPlannedOppositeState(
    args.config,
    args.state,
    args.state.market.startTs + args.secsFromOpen,
  );
  if (materialOpenPlannedOpposite) {
    return blockedPlan("xuan_material_planned_opposite_open", {
      plannedOppositeSide: materialOpenPlannedOpposite.plannedOppositeSide,
      plannedOppositeQty: materialOpenPlannedOpposite.plannedOppositeQty,
      plannedOppositeFilledQty: materialOpenPlannedOpposite.plannedOppositeFilledQty,
      plannedOppositeMissingQty: materialOpenPlannedOpposite.plannedOppositeMissingQty,
  ...
  const originalSeedExecution = seedSide === "UP" ? args.candidate.upExecution : args.candidate.downExecution;
  const originalOppositeExecution = oppositeSide === "UP" ? args.candidate.upExecution : args.candidate.downExecution;
  const postMergeReentryCompletionBlockReason = fixedFivePostMergeReentryCompletionBlockReason({
    config: args.config,
    state: args.state,
    side: seedSide,
  ...
  if (postMergeReentryCompletionBlockReason) {
    return blockedPlan(postMergeReentryCompletionBlockReason, {
      plannedOppositeSide: oppositeSide,
      plannedOppositeQty: stagedQty,
      plannedOppositeBlockedReason: postMergeReentryCompletionBlockReason,
    });
  }
  const currentMatchedPair =
    mergeableShares(args.state) > 1e-6
      ? matchedEffectivePairCost(args.state, args.config.cryptoTakerFeeRate)
      : Number.POSITIVE_INFINITY;
  const pairAlreadyCostDisciplined = xuanPairCostImprovesOrMeetsTarget(
    args.config,
    currentMatchedPair,
    args.candidate.pairCost,
  );
  const seedIsLowSide = originalSeedExecution.averagePrice <= originalOppositeExecution.averagePrice + 1e-9;
  const targetPlan = stagedPairCostPlan({
    config: args.config,
    seedPrice: originalSeedExecution.averagePrice,
    oppositePrice: originalOppositeExecution.averagePrice,
    currentEffectivePair: currentMatchedPair,
  });
  const strictSeedCostBlockReason = strictXuanSeedCostBlockReason({
    config: args.config,
    state: args.state,
    secsFromOpen: args.secsFromOpen,
    referencePairCost: args.candidate.pairCost,
  ...
  if (strictSeedCostBlockReason === "xuan_high_cost_seed_exploratory_cap") {
    return blockedPlan("xuan_strict_seed_abort_high_cost", {
      plannedOppositeSide: oppositeSide,
      plannedOppositeQty: stagedQty,
      plannedOppositeMaxPrice: normalizeTraceNumber(targetPlan.targetMaxPrice),
      plannedOppositeBlockedReason: strictSeedCostBlockReason,
    });
  } else if (strictSeedCostBlockReason) {
    return blockedPlan(
      strictSeedCostBlockReason.startsWith("xuan_high_cost_seed")
        ? "xuan_strict_seed_abort_high_cost"
        : strictSeedCostBlockReason === "xuan_late_seed_pair_cost_wait" ||
          strictSeedCostBlockReason === "xuan_post_merge_seed_pair_cost_wait" ||
          strictSeedCostBlockReason === "xuan_late_seed_deadline_after_final_merge" ||
          strictSeedCostBlockReason === "xuan_open_planned_opposite_no_closeable_path"
          ? "xuan_strict_seed_abort_uncloseable"
          : strictSeedCostBlockReason,
      {
        plannedOppositeSide: oppositeSide,
        plannedOppositeQty: stagedQty,
        plannedOppositeMaxPrice: normalizeTraceNumber(targetPlan.targetMaxPrice),
  ...
  if (stagedQty < Math.max(args.config.liveSmallLotLadder[0] ?? 80, args.state.market.minOrderSize) - 1e-9) {
    return blockedPlan("xuan_strict_seed_abort_micro_reentry", {
      plannedOppositeSide: oppositeSide,
      plannedOppositeQty: stagedQty,
    });
  }
  if (!targetPlan.viable || (!seedIsLowSide && !pairAlreadyCostDisciplined)) {
    return blockedPlan("xuan_strict_seed_abort_no_closeable_opposite_path", {
      plannedOppositeSide: oppositeSide,
      plannedOppositeQty: stagedQty,
      plannedOppositeMaxPrice: normalizeTraceNumber(targetPlan.targetMaxPrice),
    });
  }
  const seedExecution = clipExecutionQuoteForStagedSeed(originalSeedExecution, stagedQty);
  const oppositeExecution = clipExecutionQuoteForStagedSeed(originalOppositeExecution, stagedQty);
  ...
  if (args.secsFromOpen >= 276) {
    return blockedPlan("xuan_strict_seed_abort_late_deadline", {
      plannedOppositeSide: oppositeSide,
      plannedOppositeQty: stagedQty,
    });
  }
  if (args.secsFromOpen >= 250 && args.secsFromOpen + minWaitSec > finalCompletionDeadlineSec + 1e-9) {
    return blockedPlan("xuan_strict_seed_abort_late_deadline", {
      ...
      plannedOppositeDeadlineSec: finalCompletionDeadlineSec,
    });
  }
  const deadlineSec = Math.min(
    Math.max(args.secsFromOpen + minWaitSec, args.secsFromOpen + plannedOppositeDeadlineAgeSec(args.config)),
  ...
  return {
    ...
    tracePatch: {
      ...
      plannedOppositeSide: oppositeSide,
      plannedOppositeQty: stagedQty,
      plannedOppositeMissingQty: stagedQty,
      ...
      plannedOppositeMaxPrice: normalizeTraceNumber(
        Math.min(oppositeExecution.averagePrice, targetPlan.targetMaxPrice),
      ),
      mustCloseBeforeMerge: true,
...
function buildBalancedPairEntryPlan(
  ...
  const fixedFiveImmediatePairMode = fixedFiveEntryClipMax(config) !== undefined;
  const fixedFiveImmediatePairAllowed =
    fixedFiveImmediatePairMode && candidate.pairCost <= config.xuanPairSweepHardCap + 1e-9;
  const postMergeFlatImmediatePairRecycle = isPostMergeFlatImmediatePairRecycle({
    config,
    state,
  ...
  if (fixedFiveImmediatePairMode && !fixedFiveImmediatePairAllowed) {
    return {
      ...
      tracePatch: {
        ...
        skipReason: "xuan_fixed_five_pair_cycle_only",
        cycleSkippedReason: "xuan_fixed_five_pair_cycle_only",
      },
    };
  }
  ...
  const exactSameSecondDualPrior = hasExactSameSecondDualSeedPrior(state.market.slug, secsFromOpen);
  const exactSeedSequencePrior =
    config.xuanCloneMode === "PUBLIC_FOOTPRINT" &&
    resolveBundledSeedSequencePrior(state.market.slug, secsFromOpen)?.scope === "exact";
  const aggressiveHardSweepWithoutExactPrior =
    ...
    !postMergeFlatImmediatePairRecycle &&
    (candidate.mode === "XUAN_HARD_PAIR_SWEEP" || candidate.mode === "XUAN_SOFT_PAIR_SWEEP");
  if (aggressiveHardSweepWithoutExactPrior) {
    return {
      decisions: [],
      stateAfter: inventoryTraceState(state),
      tracePatch: {
        ...
        skipReason: "xuan_pair_sweep_requires_exact_prior",
        cycleSkippedReason: "xuan_pair_sweep_requires_exact_prior",
      },
    };
  }
...
function buildStagedDebtReducingFlowSeed(args: {
  ...
  const highExecution = highSide === "UP" ? args.candidate.upExecution : args.candidate.downExecution;
  ...
  const minWaitSec = plannedOppositeMinWaitSec(args.config, args.ctx.secsFromOpen, args.ctx.secsToClose);
  const finalCompletionDeadlineSec = Math.min(
    276,
    args.state.market.endTs - args.state.market.startTs - args.config.finalWindowNoChaseSec,
  );
  if (args.ctx.secsFromOpen >= 276) {
    return undefined;
  }
  if (args.ctx.secsFromOpen >= 250 && args.ctx.secsFromOpen + minWaitSec > finalCompletionDeadlineSec + 1e-9) {
    return undefined;
  }
  const deadlineSec = Math.min(
    Math.max(args.ctx.secsFromOpen + minWaitSec, args.ctx.secsFromOpen + plannedOppositeDeadlineAgeSec(args.config)),
    args.ctx.secsFromOpen >= 250 ? finalCompletionDeadlineSec : 282,
  );
  const targetPlan = stagedPairCostPlan({
    config: args.config,
    seedPrice: lowExecution.averagePrice,
    oppositePrice: highExecution.averagePrice,
    currentEffectivePair: args.basketState.basketEffectiveAvg,
  });
  return {
    decision: buildEntryBuy(
      args.state,
    ...
    plannedOppositeMaxPrice: targetPlan.targetMaxPrice,
    plannedOppositeMinWaitSec: minWaitSec,
    plannedOppositeDeadlineSec: deadlineSec,
  };
}
...
function describePairGate(
  ...
  if (allowance.rejectionReason) {
    return allowance.rejectionReason;
  }
  if (pairCost <= cap) {
    return "pair_cap";
  }
